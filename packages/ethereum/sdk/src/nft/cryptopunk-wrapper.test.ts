...
describe.skip("wrap crypto punk", () => {
  ...
  test("should wrap cryptopunk", async () => {
    ...
    try {
      const apTx = await approveForWrapper(ethereum, send, getConfig, punkId)
      ...
      console.log("skip approve", e)
    }

    const wrapTx = await wrapPunk(ethereum, send, getConfig, punkId)
  ...
  test("should unwrap cryptopunk", async () => {
    const tx = await unwrapPunk(ethereum, send, getConfig, punkId)
