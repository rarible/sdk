...
export async function approveForWrapper(
  ...
  const marketContract = createCryptoPunksMarketContract(ethereum, config.cryptoPunks.marketContract)

  const saleState = await marketContract.functionCall("punksOfferedForSale", punkIndex).call()
  ...
  if (
    ...
    return send(
      marketContract.functionCall("offerPunkForSaleToAddress", punkIndex, 0, config.cryptoPunks.wrapperContract),
    )
  }
...
export async function wrapPunk(
  ...
  const wrapperContract = createCryptoPunksWrapperContract(ethereum, config.cryptoPunks.wrapperContract)
