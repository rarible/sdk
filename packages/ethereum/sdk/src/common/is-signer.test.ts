import { personalSign } from "eth-sig-util"
import Wallet from "ethereumjs-wallet"
import { fromRpcSig, bufferToHex, toBuffer, setLengthLeft } from "ethereumjs-util"
import { randomWord } from "@rarible/types"
import { createGanacheProvider } from "@rarible/ethereum-sdk-test-common/build/create-ganache-provider"
import type { Web3Ethereum } from "@rarible/web3-ethereum"
import { isSigner } from "./is-signer"
import { createTestAdapters } from "./test/provider-adapters"
import type { AbiItem } from "./abi-item"

describe("isSigner", () => {
	const { provider, wallets } = createGanacheProvider()
	const adapters = createTestAdapters(provider, wallets[0])

	test("recover works for hw wallets workaround", async () => {
		const hash = randomWord()

		const pk = Buffer.from(randomWord().substring(2), "hex")
		const wallet = new Wallet(pk)

		const signature = personalSign(pk, { data: hash })
		const sig = fromRpcSig(signature)
		const fixed = toRpcSig(sig.v + 4, sig.r, sig.s)
		expect(await isSigner(null as any, wallet.getAddressString(), Buffer.from(hash.substring(2), "hex"), fixed))
			.toBe(true)
	})

	test.each(adapters.cases)("erc1271 works with $s", async ({ adapter }) => {
		const test = await deployTestErc1271(adapters.web3)
		const from = await adapter.getFrom()
		const hash = Buffer.from(randomWord().substring(2), "hex")
		const pk = Buffer.from(randomWord().substring(2), "hex")
		const signature = personalSign(pk, { data: randomWord() })

		await test.methods.setReturnSuccessfulValidSignature(false).send({ from })
		expect(await isSigner(adapter, test.options.address, hash, signature)).toBe(false)

		await test.methods.setReturnSuccessfulValidSignature(true).send({ from })
		expect(await isSigner(adapter, test.options.address, hash, signature)).toBe(true)
	})
})

function toRpcSig(v: number, r: Buffer, s: Buffer) {
	return bufferToHex(Buffer.concat([setLengthLeft(r, 32), setLengthLeft(s, 32), toBuffer(v)]))
}

async function deployTestErc1271(adapter: Web3Ethereum) {
	const web3 = adapter.getWeb3Instance()
	const empty = new web3.eth.Contract(testABI)
	const [address] = await web3.eth.getAccounts()
	return empty
		.deploy({ data: bytecode, arguments: [] })
		.send({ from: address, gas: 1000000, gasPrice: "0" })
}

const bytecode = ""
const testABI: AbiItem[] = [
	{
		"inputs": [],
		"name": "ERC1271_INTERFACE_ID",
		"outputs": [
			{
				"internalType": "bytes4",
				"name": "",
				"type": "bytes4",
			},
		],
		"stateMutability": "view",
		"type": "function",
	},
	{
		"inputs": [],
		"name": "ERC1271_RETURN_INVALID_SIGNATURE",
		"outputs": [
			{
				"internalType": "bytes4",
				"name": "",
				"type": "bytes4",
			},
		],
		"stateMutability": "view",
		"type": "function",
	},
	{
		"inputs": [],
		"name": "ERC1271_RETURN_VALID_SIGNATURE",
		"outputs": [
			{
				"internalType": "bytes4",
				"name": "",
				"type": "bytes4",
			},
		],
		"stateMutability": "view",
		"type": "function",
	},
	{
		"inputs": [
			{
				"internalType": "bool",
				"name": "value",
				"type": "bool",
			},
		],
		"name": "setReturnSuccessfulValidSignature",
		"outputs": [],
		"stateMutability": "nonpayable",
		"type": "function",
	},
	{
		"inputs": [
			{
				"internalType": "bytes32",
				"name": "_hash",
				"type": "bytes32",
			},
			{
				"internalType": "bytes",
				"name": "_signature",
				"type": "bytes",
			},
		],
		"name": "isValidSignature",
		"outputs": [
			{
				"internalType": "bytes4",
				"name": "",
				"type": "bytes4",
			},
		],
		"stateMutability": "view",
		"type": "function",
	},
]
