...
describe.skip("fillOrder: Opensea orders", function () {
  ...
  beforeAll(async () => {
    ...
    wyvernExchange = await deployOpenSeaExchangeV1(
      ...
      EVM_ZERO_ADDRESS, //ETH
      feeRecipient,
    )
    console.log("deployed wyvernExchange", wyvernExchange.options.address)

    await sentTx(it.exchangeWrapper.methods.__ExchangeWrapper_init(wyvernExchange.options.address, EVM_ZERO_ADDRESS), {
      from: sender1Address,
    })
    ...
    await sentTx(wyvernProxyRegistry.methods.registerProxy(), { from: sender2Address })
  ...
  async function getBalance(asset: Asset, sender: EVMAddress): Promise<string> {
    switch (asset.assetType.assetClass) {
      ...
      default:
        throw new Error("Should specify the ERC asset")
    }
  }
  ...
  test("get order signature", async () => {
    ...
    expect(signature).toBe(
      "0xa52af7ce5428c6b0ac183401849db6aea1e1d27ec26fb05dc2fb8d657c4fdd6e5002e8578fd8fee8d1647995a0f99cfbda44e22650c56d7f936feb7dcad709211b",
    )
  ...
  test("should cancel order", async () => {
    ...
    const cancelledOrder = await cancel(checkLazyOrder, ethereum1, send1, getConfig, getApis1, {
      ...order,
      signature,
    })
  ...
  ])("side: $data.side $make.assetType.assetClass for $take.assetType.assetClass", testOrder => {
