...
import { randomWord, toEVMAddress, toBigNumber, EVM_ZERO_ADDRESS } from "@rarible/types"
...
describe.skip("fillOrder: Opensea orders", function () {
  ...
  beforeAll(async () => {
    ...
    await sentTx(it.exchangeWrapper.methods.__ExchangeWrapper_init(EVM_ZERO_ADDRESS, it.exchangeV2.options.address), {
      from: sender1Address,
    })
  ...
  async function mintTestAssetAndReturnOrder(
    ...
    switch (asset.assetType.assetClass) {
      ...
      case "ERC1155": {
        await sentTx(
          it.testErc1155.methods.mint(receiver, asset.assetType.tokenId, toBn(asset.value).multipliedBy(10), "0x"),
          { from: sender },
        )
  ...
  async function getBalance(
    ...
    switch (assetType) {
      ...
      default:
        throw new Error("Should never heppen")
    }
  }
