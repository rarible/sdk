...
export class BatchOrderFiller {
  ...
  private getFillAction<Request extends FillBatchOrderRequest>(): Action<
    FillOrderStageId,
    Request,
    EthereumTransaction
  > {
  ...
  private async getTransactionRequestData(
    ...
    let gasLimit = await wrapperContract
      .functionCall(
        "bulkPurchase",
        ordersCallData,
        feeAddresses[0],
    ...
    const gasLimitWithTheshold = toBn(gasLimit).multipliedBy(1.1).integerValue(BigNum.ROUND_FLOOR).toNumber()
  ...
  private async getOrderData(preparedOrder: PreparedOrder): Promise<PreparedOrderRequestDataForExchangeWrapper> {
  ...
  return assets.reduce(
    (acc, asset) => {
      if (isErc20(asset.assetType)) {
        ...
        if (sumAsset) {
          acc.set(asset.assetType.contract, {
            ...sumAsset,
