...
export async function getMarketData(
  ...
  let valueWithOriginFees = calcValueWithFees(
    toBigNumber(fillData.options.value?.toString() ?? "0"),
    totalFeeBasisPoints,
  )
  ...
  if (request.addRoyalty && request.assetType) {
    ...
    if (royalties?.length) {
      ...
      const royaltiesAmount = getRoyaltiesAmount(royalties, fillData.options.value?.toString() ?? 0)
...
export function encodeDataWithRoyalties({
  royalties,
  data,
  provider,
}: {
  ...
  const dataForEncoding = {
    data,
    additionalRoyalties: royalties.map(royalty => encodeBasisPointsPlusAccount(royalty.value, royalty.account)),
  }
...
export async function getAmmItemsRoyalties(
  ...
  if (Array.isArray(request.assetType)) {
    ...
    return (
      (
        await apis.nftItem.getNftItemRoyaltyById({
