...
export class OrderFiller {
	...
	constructor(
		...
		private readonly getBaseOrderFee: (type: OrderData["@type"]) => Promise<number>,
		...
		this.checkLazyAssetType = checkLazyAssetType.bind(this, apis.nftItem, config.chainId)
			...
			.create({
				...
				run: async (request: Request) => {
					...
					if (request.assetType && inverted.make.type["@type"] === "COLLECTION") {
						...
						inverted.make.type = await this.checkLazyAssetType(inverted.make.type)
					}
	...
	isNonInvertableOrder(order: SimpleOrder): boolean {
		return order.data["@type"] === "ETH_BASIC_SEAPORT_DATA_V1" ||
      order.data["@type"] === "ETH_LOOKSRARE_ORDER_DATA_V1" ||
      order.data["@type"] === "ETH_LOOKSRARE_ORDER_DATA_V2" ||
      order.data["@type"] === "ETH_X2Y2_ORDER_DATA_V1" ||
