...
import { getMaximumSizeForOrder, isCriteriaItem, isCurrencyItem, isErc721Item, isNativeCurrencyItem } from "./item"
...
export const shouldUseBasicFulfill = (
  ...
  const nfts = allItems.filter(({ itemType }) => [ItemType.ERC721, ItemType.ERC1155].includes(itemType))

  const nftsWithCriteria = allItems.filter(({ itemType }) => isCriteriaItem(itemType))
...
export function validateAndSanitizeFromOrderStatus(order: Order, orderStatus: OrderStatus): Order {
...
export function generateFulfillOrdersFulfillments(ordersMetadata: FulfillOrdersMetadata): {
  ...
  ordersMetadata.forEach(({ order, offererOperator, offerCriteria }, orderIndex) => {
    const itemToCriteria = getItemToCriteriaMap(order.parameters.offer, offerCriteria)
  ...
  ordersMetadata.forEach(({ order, considerationCriteria, tips }, orderIndex) => {
    const itemToCriteria = getItemToCriteriaMap(order.parameters.consideration, considerationCriteria)
...
export const getAdvancedOrderNumeratorDenominator = (order: Order, unitsToFill: BigNumberValue) => {
  ...
  const numerator = unitsToFill ? unitsToFillBn.div(unitsGcd) : toBn(1)
