...
export async function prepareSeaportExchangeData(
  ...
  const [offererBalancesAndApprovals, fulfillerBalancesAndApprovals, orderStatusRaw] = await Promise.all([
  ...
  const sanitizedOrder = validateAndSanitizeFromOrderStatus(order, orderStatus)
