...
export class SeaportOrderHandler {
  ...
  convertOriginFeesToTips(request: SeaportV1OrderFillRequest): TipInputItem[] | undefined {
    ...
    const feeBase =
      make.assetType.assetClass === "ERC1155" && !toBn(request.amount).isEqualTo(make.value)
    ...
    return request.originFees?.map(fee => ({
      ...
      amount: feeBase.multipliedBy(toBn(fee.value)).dividedBy(10000).integerValue(BigNumberUtils.ROUND_FLOOR).toFixed(),
  ...
  async getTransactionDataForExchangeWrapper(
    ...
    return prepareSeaportExchangeData(this.ethereum, this.send.bind(this), request.order, {
  ...
  getAssetToApprove(request: SeaportV1OrderFillRequest, feesData: ComplexFeesReducedData): Asset {
    ...
    const totalPrice = toBn(take.value).div(make.value).multipliedBy(request.amount)
...
function getUnitsToFill(request: SeaportV1OrderFillRequest): {
  ...
  const unitsToFill =
    request.order.make.assetType.assetClass === "ERC1155" || request.order.take.assetType.assetClass === "ERC1155"
      ? request.amount
      : undefined
  const isSupportedPartialFill =
    request.order.data.orderType === "PARTIAL_RESTRICTED" || request.order.data.orderType === "PARTIAL_OPEN"
...
export function convertOrderType(type: SeaportOrderType): OrderType {
  switch (type) {
    case SeaportOrderType.FULL_OPEN:
      return OrderType.FULL_OPEN
    case SeaportOrderType.PARTIAL_OPEN:
      return OrderType.PARTIAL_OPEN
    case SeaportOrderType.FULL_RESTRICTED:
      return OrderType.FULL_RESTRICTED
    case SeaportOrderType.PARTIAL_RESTRICTED:
