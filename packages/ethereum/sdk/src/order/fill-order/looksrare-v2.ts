...
export class LooksrareV2OrderHandler {
  ...
  async prepareTransaction(request: LooksrareOrderV2FillRequest, originFees: Part[] | undefined) {
    ...
    const validity = await validator
      .functionCall("checkMakerOrderValidity", order, request.order.signature, merkleRoot)
      .call()
    const errorCodes = validity.filter((num: string) => !!+num).map((num: string) => OrderValidatorCode[+num])
  ...
  async prepareTransactionData(
    ...
    if (request.addRoyalty) {
      ...
      if (royaltyList?.length) {
        ...
        const royaltiesAmount = getRoyaltiesAmount(royaltyList, price ?? 0)
