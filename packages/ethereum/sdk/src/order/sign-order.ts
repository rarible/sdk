import type { Asset, Binary } from "@rarible/ethereum-api-client"
  ...
  switch (order.type) {
    ...
    case "RARIBLE_V2": {
      ...
      if (!signature || typeof signature !== "string") {
        throw new Error(
          `signOrder error: signature=${getStringifiedData(signature)}, data=${JSON.stringify(structMessage)}`,
        )
      }
...
function createEIP712Domain(chainId: number, verifyingContract: string): EIP712Domain {
  return {
...
export function orderToStruct(ethereum: Ethereum, order: SimpleRaribleV2Order, wrongEncode: Boolean = false): any {
  ...
  return {
    ...
    taker: order.taker ?? EVM_ZERO_ADDRESS,
