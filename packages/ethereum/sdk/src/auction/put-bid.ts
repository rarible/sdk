...
export class PutAuctionBid {
	...
	readonly putBid: PutAuctionBidAction = Action.create({
		...
		run: async (request: PutBidRequest) => {
			...
			const price = await getPrice(wallet, auction.buy, request.priceDecimal)
			const priceBn = toBigNumber(price.toString())

			if (auction.buy.assetClass !== "ETH") {
				const from = await wallet.getFrom()
				const asset = { assetType: auction.buy, value: priceBn }
				const approveTx = await this.approve(toAddress(from), asset, true)
				if (approveTx) {
					// Wait for approval to be confirmed before actual bid
					await approveTx.wait()
				}
			}
			return { request, auction, price: priceBn }
		},
	})
		.thenStep({
			id: "sign" as const,
