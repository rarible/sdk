  ...
  readonly start: AuctionStartAction = Action.create({
    ...
    run: async ({ request, makeAssetType }: { request: CreateAuctionRequest; makeAssetType: AssetTypeResponse }) => {
      ...
      const tx = await this.send(
        await createEthereumAuctionContract(this.ethereum, config.auction).functionCall(
          "startAuction",
          sellAsset,
          buyAssetType,
      ...
      const auctionIdPromise = tx.getEvents().then(async events => {
      ...
      const hashPromise = auctionIdPromise.then(auctionId => {
