...
describe.skip("finish auction auction", () => {
  const { provider: providerSeller, wallet: walletSeller } = createE2eProvider(
    "0xded057615d97f0f1c751ea2795bc4b03bbf44844c13ab4f5e6fd976506c276b9",
  )
  const { provider: providerBuyer, wallet: walletBuyer } = createE2eProvider(
    "0xa0d2baba419896add0b6e638ba4e50190f331db18e3271760b12ce87fa853dcb",
  )
  ...
  test("finish auction erc-1155 <-> erc-20", async () => {
    ...
    await sentTx(it.testErc20.methods.mint(sender2Address, erc20Supply.toString()), {
      from: sender1Address,
      gas: 1000000,
    })
    await sentTx(it.testErc20.methods.mint(sender1Address, "10000000000000000000000000000"), {
      from: sender1Address,
      gas: 1000000,
    })
    ...
    expect(await it.testErc20.methods.balanceOf(sender2Address).call()).toBe(finalBidderErcBalance.toString())
