      ...
      const modifyParams = (cod: typeof coder, param: any) => {
        if (cod.name === "array") {
          return param.map((p: any) =>
            modifyParams(ethersAbiCoder._getCoder(ParamType.from(cod.type.replace("[]", ""))), p),
          )
        }
        // @ts-ignore
        const { coders } = cod
...
function mapTypes(types: any[]) {
  ...
  types.forEach(function (type) {
    ...
    if (isSimplifiedStructFormat(type)) {
      ...
      mappedTypes.push(
        Object.assign(mapStructNameAndType(structName), {
...
function mapStructToCoderFormat(struct: { [index: string]: any }): any[] {
  ...
  Object.keys(struct).forEach(function (key) {
    if (typeof struct[key] === "object") {
      components.push(
        Object.assign(mapStructNameAndType(key), {
