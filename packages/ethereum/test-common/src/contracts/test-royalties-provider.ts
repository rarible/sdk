import type { Web3 } from "web3"
import type { EVMAddress } from "@rarible/ethereum-api-client"
import { DEFAULT_DATA_TYPE, replaceBigIntInContract } from "../common"

export function createTestRoyaltiesProviderContract(web3: Web3, address?: EVMAddress) {
  return new web3.eth.Contract(testRoyaltiesProviderAbi, address, DEFAULT_DATA_TYPE)
}

export async function deployTestRoyaltiesProvider(web3: Web3) {
  const empty = createTestRoyaltiesProviderContract(web3)
  const [address] = await web3.eth.getAccounts()
  const contract = await empty.deploy({ data: testRoyaltiesProviderBytecode }).send({ from: address, gas: "4000000" })
  return replaceBigIntInContract(contract)
}

const testRoyaltiesProviderAbi = [
  {
    inputs: [
      {
        internalType: "address",
        name: "token",
        type: "address",
      },
      {
        internalType: "uint256",
        name: "tokenId",
        type: "uint256",
      },
    ],
    name: "getRoyalties",
    outputs: [
      {
        components: [
          {
            internalType: "address payable",
            name: "account",
            type: "address",
          },
          {
            internalType: "uint96",
            name: "value",
            type: "uint96",
          },
        ],
        internalType: "struct LibPart.Part[]",
        name: "",
        type: "tuple[]",
      },
    ],
    stateMutability: "nonpayable",
    type: "function",
  },
] as const

export const testRoyaltiesProviderBytecode =
  ""
