...
import {} from "../sdk-blockchains/ethereum/test/common"
...
describe("Logging", () => {
  ...
  test("Should log simple blockchain call", async () => {
    ...
    expect(Object.keys(logger.raw.mock.calls[0][0])).toEqual(["level", "method", "message", "duration", "args", "resp"])
  ...
  test.skip("Should handle error simplified blockchain call", async () => {
    ...
    try {
      const tx = await sdk.order.buy({
        orderId: toOrderId("ETHEREUM:0x78dafa455051f8b7a1abcab8d028a7ef0a9e5c8db053176f826ba12e81b87292"),
        amount: 1,
      })
      await tx.wait()
    } catch (e) {}
