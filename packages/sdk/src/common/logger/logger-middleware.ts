    ...
    case WalletType.ETHEREUM: {
      ...
      info["wallet.address"] =
        addressResult.status === "fulfilled"
          ? addressResult?.value?.toLowerCase()
...
export function getInternalLoggerMiddleware(
  ...
  const remoteLogger =
    externalLogger ??
    new RemoteLogger((msg: LoggableValue) => axios.post(loggerConfig.elkUrl, msg), {
...
export const HUMAN_READABLE_MSG_LIST: Record<string, string> = {
  [INVALID_TX_PARAMS_EIP_1559_ERROR]:
    "The problem is with the selected network of your wallet provider, try switching the network or re-entering the wallet. You can read more about this error [here](https://github.com/MetaMask/metamask-extension/issues/13341)",
}
