...
export class ItemTestSuite<T extends SuiteSupportedBlockchain> {
  ...
  mintAndWait = async (collectionId: CollectionId, override: Partial<MintSimplifiedRequestOnChain> = {}) => {
  ...
  waitItem = (itemId: ItemId, supply: number | undefined) => {
    return waitFor(
      () => this.sdk.apis.item.getItemById({ itemId }),
      x => {
  ...
  mintLazyAndWait = async (collectionId: CollectionId, override: Partial<MintSimplifiedRequestOffChain> = {}) => {
    const common = await this.createDefaultMintValues(this.blockchain, collectionId)
  ...
  private async createDefaultMintValues<T extends SuiteSupportedBlockchain>(blockchain: T, collectionId: CollectionId) {
