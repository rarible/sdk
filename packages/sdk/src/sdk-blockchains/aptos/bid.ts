...
import { APT_DIVIDER } from "@rarible/aptos-sdk/build/common"
import type { OrderId } from "@rarible/api-client"
import type { IBlockchainTransaction } from "@rarible/sdk-transaction"
import { BlockchainAptosTransaction } from "@rarible/sdk-transaction"
import type {
	FillRequest,
	PrepareFillRequest, PrepareFillResponse } from "../../types/order/fill/domain"
import {
...
export class AptosBid {
	...
	async bid(prepare: PrepareBidRequest): Promise<PrepareBidResponse> {
		const item = "itemId" in prepare
			? await this.apis.item.getItemById({ itemId: prepare.itemId })
			: null
			...
			.create({
				...
				run: async (buyRequest: FillRequest) => {
					...
					if (order.take.type["@type"] === "NFT_OF_COLLECTION") {
						...
						const tx = await this.sdk.order.acceptCollectionOffer(
							extractId(orderId),
							extractId(buyRequest.itemId)
						)
						return new BlockchainAptosTransaction(tx, this.network, this.sdk)
					}
					if (order.take.type["@type"] === "NFT") {
						const tx = await this.sdk.order.acceptTokenOffer(
							extractId(orderId)
						)
						return new BlockchainAptosTransaction(tx, this.network, this.sdk)
					}
