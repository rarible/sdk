...
export class AptosBid {
  ...
  async bid(prepare: PrepareBidRequest): Promise<PrepareBidResponse> {
    ...
    const prepareCollectionId = "collectionId" in prepare ? toUnionContractAddress(prepare.collectionId) : undefined
    ...
    return {
      ...
      submit,
      nftData: {
        nftCollection: item?.collection ? toUnionContractAddress(item.collection) : prepareCollectionId,
      },
    }
  }
