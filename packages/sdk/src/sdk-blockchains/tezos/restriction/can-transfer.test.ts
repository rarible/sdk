...
describe("canTransfer", () => {
  ...
  test.skip("returns false and reason for whitelisted collection", async () => {
    ...
    const result = await sdk.restriction.canTransfer(
      toItemId("TEZOS:KT1S3goQNhyuZgznN952Vwfqeo96YV3U4pwf:100005"),
      me,
      otherMe,
    )
    expect(result).toStrictEqual({
      success: false,
      reason:
        "You can't trade this Digit at the moment, please visit [quartz.ubisoft.com](https://quartz.ubisoft.com) for more information.",
    })
  })
  ...
  test.skip("returns true for whitelisted addresses", async () => {
    ...
    const result = await sdk.restriction.canTransfer(
      toItemId("TEZOS:KT1S3goQNhyuZgznN952Vwfqeo96YV3U4pwf:100002"),
      from,
      to,
    )
    expect(result).toStrictEqual({
...
describe.skip("canTransfer for quartz prod contract", () => {
  ...
  test("not whitelisted quartz wallet should restricted", async () => {
    ...
    const result = await sdk.restriction.canTransfer(
      toItemId("TEZOS:KT1TnVQhjxeNvLutGvzwZvYtC7vKRpwPWhc6:800020"),
      me,
      otherMe,
    )
    expect(result).toStrictEqual({
