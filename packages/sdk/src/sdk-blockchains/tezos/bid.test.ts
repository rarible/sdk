...
describe.skip("bid test", () => {
	...
	const it = awaitAll({
		itemOwnerSdk: createRaribleSdk(itemOwner, env, { logs: LogsLevel.DISABLED }),
		bidderSdk: createRaribleSdk(bidderWallet, env, { logs: LogsLevel.DISABLED }),
		nullFundsWalletSdk: createRaribleSdk(nullFundsWallet, env, { logs: LogsLevel.DISABLED }),
	})
	...
	test("bid NFT test", async () => {
		const mintResponse = await it.itemOwnerSdk.nft.mint.prepare({
		...
		await awaitItemSupply(it.itemOwnerSdk, mintResult.itemId, "10")

		const bidResponse = await it.bidderSdk.order.bid.prepare({ itemId: mintResult.itemId })
		...
		await awaitForOrder(it.bidderSdk, orderId)

		// update bid price
		const updateAction = await it.bidderSdk.order.bidUpdate.prepare({ orderId })
		const updatedBidOrderId = await updateAction.submit({ price: "0.000002" })

		await retry(10, 1000, async () => {
			const order = await it.bidderSdk.apis.order.getOrderById({
		...
		const acceptBidResponse = await it.itemOwnerSdk.order.acceptBid.prepare({ orderId: updatedBidOrderId })
		...
		await awaitForOrderStatus(it.bidderSdk, updatedBidOrderId, "FILLED")
	}, 1500000)

	test("bid NFT test with basic function", async () => {
		const mintResult = await it.itemOwnerSdk.nft.mint({
			collectionId: convertTezosToCollectionAddress(nftContract),
			uri: "ipfs://bafkreiaz7n5zj2qvtwmqnahz7rwt5h37ywqu7znruiyhwuav3rbbxzert4",
		})
		await mintResult.transaction.wait()

		await awaitItemSupply(it.itemOwnerSdk, mintResult.itemId, "1")

		const orderId = await it.bidderSdk.order.bid({
		...
		// update bid price
		await it.bidderSdk.order.bidUpdate({
			orderId,
			price: "0.000004",
		})
		...
		// accept bid by item owner
		const fillBidResult = await it.itemOwnerSdk.order.acceptBid({
			orderId,
			amount: 1,
		})
		await fillBidResult.wait()

		await awaitForOrderStatus(it.bidderSdk, orderId, "FILLED")
	}, 1500000)
	...
	test("bid MT test", async () => {
		...
		const startBalanceFeesWallet = await it.itemOwnerSdk.balances.getBalance(originFeeWallet, {
			"@type": "TEZOS_FT",
		...
		// accept bid by item owner
		const acceptBidResponse = await it.itemOwnerSdk.order.acceptBid.prepare({ orderId })
		const fillBidResult = await acceptBidResponse.submit({
			amount: 3,
			infiniteApproval: true,
			originFees: [{
		...
		const finishBalanceFeesWallet = await it.itemOwnerSdk.balances.getBalance(originFeeWallet, {
			"@type": "TEZOS_FT",
			contract: eurTzContract,
			tokenId: toBigNumber("0"),
		})
	...
	test.skip("bid MT test with CurrencyId", async () => {
		...
		const order = await awaitForOrder(it.bidderSdk, orderId)
	...
	test.skip("getConvertValue returns insufficient type", async () => {
		...
		const bidResponse = await it.nullFundsWalletSdk.order.bid.prepare({ itemId: mintResult.itemId })
	...
	test.skip("getConvertValue returns undefined when passed non-wXTZ contract", async () => {
		...
		await awaitItemSupply(it.itemOwnerSdk, mintResult.itemId, "5")
	...
	test.skip("convert currency on bid", async () => {
		...
		await awaitItemSupply(it.itemOwnerSdk, mintResult.itemId, "4")
		...
		const updateAction = await it.bidderSdk.order.bidUpdate.prepare({ orderId: bidOrderId })
		await updateAction.submit({ price: "0.00004" })

		const acceptBidResponse = await it.itemOwnerSdk.order.acceptBid.prepare({ orderId: bidOrderId })
