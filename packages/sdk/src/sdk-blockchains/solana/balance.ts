...
export class SolanaBalance {
  ...
  async getBalance(address: UnionAddress, currency: RequestCurrency): Promise<BigNumberValue> {
    ...
    if (assetType["@type"] === "SOLANA_SOL") {
      ...
      return await this.sdk.balances.getTokenBalance(extractPublicKey(address), extractPublicKey(assetType.itemId))
    } else {
  ...
  depositBiddingBalance = Action.create({
    id: "send-tx" as const,
  ...
  withdrawBiddingBalance = Action.create({
    id: "send-tx" as const,
