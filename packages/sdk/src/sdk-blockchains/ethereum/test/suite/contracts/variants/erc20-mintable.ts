...
export class ERC20Mintable<T extends EVMSuiteSupportedBlockchain> extends ERC20<T> {
  constructor(blockchain: T, addressString: string, provider: EVMSuiteProvider<T>) {
    super(createErc20Contract(provider, toEVMAddress(addressString)), blockchain, addressString, provider)
  }
  ...
  static async deploy<T extends EVMSuiteSupportedBlockchain>(blockchain: T, provider: EVMSuiteProvider<T>) {
