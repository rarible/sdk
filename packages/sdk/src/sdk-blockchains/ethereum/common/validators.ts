...
export function validateOrderDataV3Request(
  request: OrderCommon.OrderRequest | FillRequest,
  options?: {
  ...
  if (
    ...
    (!request.maxFeesBasePoint || request.maxFeesBasePoint <= 0 || request.maxFeesBasePoint > 1000)
  ) {
    throw new Warning(
      "maxFeesBasePoint should be specified in request and should be more than 0% and can't be more than 10%",
    )
  }
