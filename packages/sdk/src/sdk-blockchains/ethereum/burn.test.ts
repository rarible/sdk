...
describe("burn", () => {
	...
	test("burn erc721", async () => {
		...
		const collection = await it.sdk.apis.collection.getCollectionById({ collection: `ETHEREUM:${contractErc721}` })
		const mintAction = await it.sdk.nft.mint.prepare({ collection })
		...
		const burn = await it.sdk.nft.burn.prepare({ itemId: mintResult.itemId })
	...
	test("burn erc1155", async () => {
		...
		await awaitItemSupply(it.sdk, mintResult.itemId, toBigNumber("10"))

		const burn = await it.sdk.nft.burn.prepare({
	...
	test.skip("burn erc-721 lazy item", async () => {
		...
		await awaitItemSupply(it.sdk, mintResult.itemId, toBigNumber("1"))
		...
		await awaitDeletedItem(it.sdk,  mintResult.itemId)
	})
