...
export class SolanaKeypairWallet implements SolanaSigner {
  ...
  signTransaction = (tx: TransactionOrVersionedTransaction) =>
    isTransaction(tx)
      ? Promise.resolve(signWithKeypair(tx, this.keyPair))
      : Promise.reject(new UnsupportedSolanaTransactionType())

  signAllTransactions = (txs: TransactionOrVersionedTransaction[]) => Promise.all(txs.map(x => this.signTransaction(x)))

  signMessage = async (message: Uint8Array | string): Promise<SolanaSignature> => ({
    publicKey: this.publicKey,
    signature: nacl.sign(getSignMessageData(message), this.keyPair.secretKey).slice(0, nacl.sign.signatureLength),
  })
}
