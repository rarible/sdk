...
function ItemMedia({ url }: { url: string | null }) {
  if (!url) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" sx={{ height: 194 }}>
...
export function ItemCard({ item }: IItemCardProps) {
  return (
    <Card sx={{ width: 200 }}>
      ...
      <CardActions>
        <Button size="small" component={Link} to={`/sell/${item.id}`}>
          Sell
        </Button>
        <IconButton size="small" component={Link} to={`/transfer/${item.id}`} title="Transfer">
          <SendIcon />
        </IconButton>
        <IconButton size="small" color={"warning"} component={Link} to={`/burn/${item.id}`} title="Burn">
          <DeleteIcon />
        </IconButton>
      </CardActions>
    </Card>
