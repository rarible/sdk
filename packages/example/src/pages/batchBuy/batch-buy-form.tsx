            ...
            onComplete(
              await prepare.submit(
                prepare.prepared.map(prepare => {
                  ...
                  let itemId: any[] = new Array(itemsCounter).fill(0).map((a, i) => {
                    console.log(
                      "maps, index",
                      prepare.orderId + `_itemId_${i}`,
                      formData[prepare.orderId + `_itemId_${i}`],
                    )
                    return formData[prepare.orderId + `_itemId_${i}`]
                      ? toItemId(formData[prepare.orderId + `_itemId_${i}`])
                      : undefined
                  })
      ...
      >
        <Stack spacing={2}>
          {prepare.prepared.map((prepare, i) => {
            return (
              <Box key={prepare.orderId + i}>
                <p>OrderId: {prepare.orderId}</p>
