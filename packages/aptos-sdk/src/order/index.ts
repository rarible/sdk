...
export class AptosOrder extends AptosMethodClass {
  sell = async (tokenAddress: string, feeObjectAddress: string, startTime: number, price: string) => {
    ...
    const accountInfo = await getRequiredWallet(this.wallet).getAccountInfo()
    const pendingTransaction = await this.aptos.transaction.build.simple({
      ...
      data: {
        ...
        functionArguments: [tokenAddress, feeObjectAddress, startTime, price],
      },
    })

    const tx = await this.sendAndWaitTx(pendingTransaction)
    const change = tx.changes.find(change => isChangeBelongsToType(change, type => type.includes("token::Token")))
  ...
  collectionOffer = async (
    ...
    const transaction = await this.aptos.transaction.build.simple({
      ...
      data: {
        ...
        typeArguments: ["0x1::aptos_coin::AptosCoin"],
        functionArguments: [collectionAddress, feeObjectAddress, price, amount, endTime],
      },
    })
