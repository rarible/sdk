...
describe("bid nft", () => {
	...
	test("create collection offer", async () => {
		...
		const {
			collectionAddress,
		} = await createTestCollectionAndMint(sellerState)

		const expirationTime = Math.floor(Date.now() / 1000) + 24 * 60 * 60 * 60

		const amount = 1
		const tx = await sellerOrderClass.collectionOffer(
			collectionAddress,
			amount,
			feeAddress,
			expirationTime,
	...
	test("cancel collection offer", async () => {
		...
		const amount = 1
		const offer = await sellerOrderClass.collectionOffer(
			collectionAddress,
			amount,
			feeAddress,
			expirationTime,
		...
		const tx = await sellerOrderClass.cancelCollectionOffer(
			offer,
		)
	...
	test("accept collection offer", async () => {
		...
		const amount = 1
		const offer = await buyerOrderClass.collectionOffer(
			collectionAddress,
			amount,
			feeAddress,
			expirationTime,
		...
		const tx = await sellerOrderClass.acceptCollectionOffer(
			offer,
			tokenAddress
		)
		expect(tx.hash).toBeTruthy()
	...
	test("create item offer", async () => {
		...
		const tokenOffer = await sellerOrderClass.tokenOffer(
			tokenAddress,
			feeAddress,
			expirationTime,
			price
	...
	test("accept item offer", async () => {
		...
		const offer = await buyerOrderClass.tokenOffer(
			tokenAddress,
			feeAddress,
			expirationTime,
			price
		...
		const tx = await sellerOrderClass.acceptTokenOffer(
			offer,
		)
		expect(tx.hash).toBeTruthy()
	})
