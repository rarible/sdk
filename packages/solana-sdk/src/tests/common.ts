...
export async function requestSol(connection: Connection, publicKey: PublicKey, sol: number = 2): Promise<number> {
  const fromAirdropSignature = await connection.requestAirdrop(publicKey, sol * LAMPORTS_PER_SOL)
...
export async function mintToken({
  ...
  expect((await sdk.balances.getTokenBalance(wallet.publicKey, mintPrepare.mint)).toString()).toEqual(
