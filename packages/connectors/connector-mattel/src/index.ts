...
export class MattelConnectionProvider extends AbstractConnectionProvider<
  typeof PROVIDER_ID,
  MattelProviderConnectionResult
> {
  ...
  constructor(private config: MattelConnectorConfig) {
    super()
  ...
  private async _connect(): Promise<ConnectionResult> {
    const [{ Magic }, { FlowExtension }, { OpenIdExtension }, fcl, auth0JS] = await Promise.all([
      import("magic-sdk"),
    ...
    fcl.config().put("accessNode.api", this.config.accessNode).put("env", this.config.network)
  ...
  async sardinePurchase(data: { orderId: OrderId; orderMaker: UnionAddress; purchaseOptions: SardinePurchaseOptions }) {
...
export type SardinePurchaseOptions = {
  nft: Omit<NFTPurchaseRequest["nft"], "blockchainNftId" | "contractAddress" | "network" | "platform" | "type">
