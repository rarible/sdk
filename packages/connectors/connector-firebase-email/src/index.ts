...
const PROVIDER_ID = "firebase_email" as const

export class FirebaseEmailConnectionProvider extends AbstractConnectionProvider<
  typeof PROVIDER_ID,
EthereumProviderConnectionResult
> {
  ...
  private async _connect(): Promise<Web3AuthNoModal> {
  	...
  	try {
  		...
  		let cachedUser = window.localStorage.getItem("firebaseEmailUser")
  		let user
  		...
  		if (cachedUser) {
  			...
  			if (email) {
  				const signInResult = await signInWithEmailLink(auth, email)
  		...
  		try {
  			...
  			window.localStorage.setItem("firebaseEmailUser", JSON.stringify(user))
  			return web3auth
  		} catch (error) {
  			return web3auth
  		}
  ...
  private async disconnect(instance: Web3AuthNoModal): Promise<void> {
  	...
  	window.localStorage.removeItem("firebaseEmailUser")
  }
}
