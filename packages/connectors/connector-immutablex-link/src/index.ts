...
export class ImmutableXLinkConnectionProvider extends AbstractConnectionProvider<
  typeof PROVIDER_ID,
  IImmutableXProviderConnectionResult
> {
  ...
  private toConnectState(wallet: ImxWallet): Observable<ConnectionState<IImmutableXProviderConnectionResult>> {
    return combineLatest([this.getAddress(wallet), this.getConnectedStatus(wallet)]).pipe(
