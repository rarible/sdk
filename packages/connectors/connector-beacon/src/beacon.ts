...
import { AbstractConnectionProvider, cache, getStateConnected, getStateConnecting } from "@rarible/connector"
...
export class BeaconConnectionProvider extends AbstractConnectionProvider<
  typeof PROVIDER_ID,
  TezosProviderConnectionResult<BeaconWallet>
> {
  ...
  private toConnectState(
    ...
    return defer(() => this.getAddress(beaconWallet)).pipe(
      map(address =>
        getStateConnected<TezosProviderConnectionResult<BeaconWallet>>({
          connection: {
            address,
