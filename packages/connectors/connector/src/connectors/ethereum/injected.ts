...
export class InjectedWeb3ConnectionProvider extends AbstractConnectionProvider<
  DappType,
  EthereumProviderConnectionResult
> {
  private readonly connection: Observable<ConnectionState<EthereumProviderConnectionResult>>

  constructor(private readonly config: InjectedWeb3ConnectionConfig = { prefer: [] }) {
    super()
  ...
  async isConnected(): Promise<boolean> {
    ...
    if (provider !== undefined) {
      return ethAccounts(provider).then(([account]) => account !== undefined)
    } else {
