...
export class ImxTradesControllerApi extends BaseAPI {
	...
	async getTradesRaw(
		...
		const response = await this.request({
			path: "/trades?party_b_token_type=" + encodeURIComponent(requestParameters.tokenType) +
			"&party_b_token_address=" + encodeURIComponent(requestParameters.tokenAddress) +
			"&party_b_token_id=" + encodeURIComponent(requestParameters.tokenId),
			method: "GET",
