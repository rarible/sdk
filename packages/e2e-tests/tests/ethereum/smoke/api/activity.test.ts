...
describe.each(suites())("$blockchain api => activity", suite => {
  ...
  test("activity controller", async () => {
    ...
    await awaitExpected(async () => {
      const activitiesByCollection = await getActivitiesByCollection(sellerSdk, suite.collectionId, [ActivityType.MINT])
      expect(activitiesByCollection.activities.length).toBeGreaterThanOrEqual(1)
    })
    await awaitExpected(async () => {
      const activitiesByCollectionRaw = (await getActivitiesByCollectionRaw(sellerSdk, suite.collectionId, [
        ActivityType.MINT,
      ])) as GetActivitiesByCollection200
    ...
    await awaitExpected(async () => {
      const activitiesByItemRaw = (await getActivitiesByItemRaw(sellerSdk, nft.id, [
        ActivityType.MINT,
      ])) as GetActivitiesByItem200
      expect(activitiesByItemRaw.value.activities.length).toBeGreaterThanOrEqual(1)
    })
    await awaitExpected(async () => {
      const activitiesByUser = await getActivitiesByUser(
        sellerSdk,
        [walletAddressSeller.unionAddress],
        [UserActivityType.MINT],
      )
      expect(activitiesByUser.activities.length).toBeGreaterThanOrEqual(1)
    })
    await awaitExpected(async () => {
      const activitiesByUserRaw = (await getActivitiesByUserRaw(
        sellerSdk,
        [walletAddressSeller.unionAddress],
        [UserActivityType.MINT],
      )) as GetActivitiesByUser200
      expect(activitiesByUserRaw.value.activities.length).toBeGreaterThanOrEqual(1)
    })
    await awaitExpected(async () => {
      const allActivities = await getAllActivities(sellerSdk, [suite.blockchain], [ActivityType.MINT])
      expect(allActivities.activities.length).toBeGreaterThanOrEqual(1)
    })
    await awaitExpected(async () => {
      const allActivitiesRaw = (await getAllActivitiesRaw(
        sellerSdk,
        [suite.blockchain],
        [ActivityType.MINT],
      )) as GetAllActivities200
