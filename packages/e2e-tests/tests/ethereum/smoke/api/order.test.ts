...
describe.each(suites())("$blockchain api => order", suite => {
  ...
  test("order controller", async () => {
    ...
    const ordersAllRaw = (await getOrdersAllRaw(sellerSdk, [suite.blockchain], 2)) as GetOrdersAll200
    ...
    const sellOrdersRaw = (await getSellOrdersRaw(sellerSdk, [suite.blockchain], 2)) as GetSellOrders200
    ...
    const sellOrdersByItemRaw = (await getSellOrdersByItemRaw(
      sellerSdk,
      nft.contract!,
      nft.tokenId!,
      2,
    )) as GetSellOrdersByItem200
    ...
    const sellOrdersByMakerRaw = (await getSellOrdersByMakerRaw(
      sellerSdk,
      walletAddressSeller.unionAddress,
      2,
    )) as GetSellOrdersByMaker200
    ...
    await retry(40, 3000, async () => {
      const orderBidsByItemRaw = (await getOrderBidsByItemRaw(
        sellerSdk,
        nft.contract!,
        nft.tokenId!,
        2,
      )) as GetOrderBidsByItem200
