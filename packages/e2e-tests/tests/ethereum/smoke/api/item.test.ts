...
describe.each(suites())("$blockchain api => ownership", suite => {
  ...
  test("item controller", async () => {
    ...
    await awaitExpected(async () => {
      const itemsByCollectionRaw = (await getItemsByCollectionRaw(sdk, collection.id, 2)) as GetItemsByCollection200
    ...
    await awaitExpected(async () => {
      const itemsByCreatorRaw = (await getItemsByCreatorRaw(sdk, address.unionAddress, 2)) as GetItemsByCreator200
    ...
    await awaitExpected(async () => {
      const itemsByOwnerRaw = (await getItemsByOwnerRaw(sdk, address.unionAddress, 2)) as GetItemsByOwner200
    ...
    await awaitExpected(async () => {
      const itemRoyaltiesRaw = (await getItemRoyaltiesByIdRaw(
        sdk,
        nft.contract!,
        nft.tokenId!,
      )) as GetItemRoyaltiesById200
