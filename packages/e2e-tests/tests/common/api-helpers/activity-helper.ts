...
import type {
  ...
  GetActivitiesByItemResponse,
  GetActivitiesByUserResponse,
...
export async function getActivitiesByCollectionRaw(
  sdk: IRaribleSdk,
  collection: string,
  activityTypes: Array<ActivityType>,
): Promise<GetActivitiesByCollectionResponse> {
  const activities = await retry(10, 2000, async () => {
...
export async function getActivitiesByItem(
  sdk: IRaribleSdk,
  itemId: string,
  activityTypes: Array<ActivityType>,
  shouldPresent?: Array<ActivityType>,
): Promise<Activities> {
...
export async function getActivitiesByItemRaw(
  sdk: IRaribleSdk,
  itemId: string,
  activityTypes: Array<ActivityType>,
): Promise<GetActivitiesByItemResponse> {
  const activities = await retry(10, 2000, async () => {
...
export async function getActivitiesByUserRaw(
  sdk: IRaribleSdk,
  user: Array<string>,
  type: Array<UserActivityType>,
): Promise<GetActivitiesByUserResponse> {
  const activities = await retry(10, 2000, async () => {
...
export async function getAllActivitiesRaw(
  sdk: IRaribleSdk,
  blockchains: Array<Blockchain>,
  type: Array<ActivityType>,
