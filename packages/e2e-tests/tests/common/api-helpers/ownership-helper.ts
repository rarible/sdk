...
export async function getOwnershipById(
  sdk: IRaribleSdk,
...
export async function getOwnershipByIdRaw(
  sdk: IRaribleSdk,
  itemId: ItemId,
  recipientAddress: string,
): Promise<GetOwnershipByIdResponse> {
  const ownershipId = `${itemId}:${recipientAddress}`
  const ownership = await retry(10, 2000, async () => {
...
export async function getOwnershipsByItem(
  sdk: IRaribleSdk,
...
export async function getOwnershipsByItemRaw(
  sdk: IRaribleSdk,
  contract: ContractAddress,
  tokenId: BigNumber,
): Promise<GetOwnershipsByItemResponse> {
  const ownershipId = `${contract}:${tokenId}`
  const ownerships = await retry(10, 2000, async () => {
