...
export async function getOrderBidsByItem(
  sdk: IRaribleSdk,
...
export async function getOrderBidsByItemRaw(
  sdk: IRaribleSdk,
  contract: ContractAddress,
  tokenId: BigNumber,
  size: number,
): Promise<GetOrderBidsByItemResponse> {
  const itemId = `${contract}:${tokenId}`
  const orders = await retry(10, 2000, async () => {
...
export async function getOrdersAll(sdk: IRaribleSdk, blockchains: Array<Blockchain>, size: number): Promise<Orders> {
  const orders = await retry(10, 2000, async () => {
...
export async function getOrdersAllRaw(
  sdk: IRaribleSdk,
...
export async function getSellOrdersRaw(
  sdk: IRaribleSdk,
  blockchains: Array<Blockchain>,
  size: number,
...
export async function getSellOrdersByItem(
  sdk: IRaribleSdk,
  contract: ContractAddress,
  tokenId: BigNumber,
  size: number,
...
export async function getSellOrdersByItemRaw(
  sdk: IRaribleSdk,
  contract: ContractAddress,
  tokenId: BigNumber,
  size: number,
): Promise<GetSellOrdersByItemResponse> {
  const itemId = `${contract}:${tokenId}`
  const orders = await retry(10, 2000, async () => {
...
export async function getSellOrdersByMakerRaw(
  sdk: IRaribleSdk,
  maker: string,
  size: number,
): Promise<GetSellOrdersByMakerResponse> {
  const orders = await retry(10, 2000, async () => {
