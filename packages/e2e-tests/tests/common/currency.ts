...
export async function getCurrency(
  ...
  if (wallets.seller instanceof EthereumWallet && wallets.buyer instanceof EthereumWallet) {
    if (currency === "ERC20") {
      ...
      const testErc20 = getTestErc20Contract(sellerWeb3, toAddress(testsConfig.variables.ETHEREUM_ERC20))
      ...
      await new Promise((resolve, reject) => {
        promiEvent.once("confirmation", (confNumber: number, receipt: EthereumTransactionReceipt) =>
