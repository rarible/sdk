...
import type { FlowNetwork, FlowTransaction } from "@rarible/flow-sdk"
...
export class BlockchainFlowTransaction implements IBlockchainTransaction {
  ...
  constructor(
    public transaction: FlowTransaction,
    public network: FlowNetwork,
  ) {}
  ...
  getTxLink() {
    switch (this.network) {
      case "mainnet":
        return `https://flowscan.org/transaction/${this.hash()}`
      case "testnet":
        return `https://testnet.flowscan.org/transaction/${this.hash()}`
      case "emulator":
        return ""
      default:
